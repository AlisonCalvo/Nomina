...
export class LeerCuentaCobroComponent implements OnInit {
   ...
   * @param cuentacobroService Servicio para gestionar la entidad
   ...
   */
  onCreate(): void {
    const dialogRef = this.dialog.open(CrearCuentaCobroComponent, {
      minWidth: '800px',
      data: {}
    });
   ...
   * @param {any} cuentacobro - El registro a editar
   ...
   * @param {number} cuentacobroId - ID del registro a eliminar
  ...
  onDelete(cuentacobroId: number) {
    ...
    const shouldProceed = this.preDelete(cuentacobroId);
    if (!shouldProceed) {
      return;
    }
    try {
  ...
  exportToExcel(): void {
    try {
      ...
      XLSX.writeFile(wb, 'cuentacobro_datos.xlsx');

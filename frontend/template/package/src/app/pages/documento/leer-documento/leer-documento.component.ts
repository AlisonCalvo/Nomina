...
import { ActualizarDocumentoComponent } from '../../documento/actualizar-documento/actualizar-documento.component';
...
@Component({
  ...
  styleUrls: ['./leer-documento.component.scss']
})
export class LeerDocumentoComponent implements OnInit {
   ...
   * @param documentoService Servicio para gestionar la entidad
  ...
  loadData(): void {
    this.documentoService.findAll().subscribe({
      next: data => {
        this.dataSource.data = data.map(item => Object.assign(new DocumentoComponent(this.documentoService), item));
   ...
   * @param {any} documento - El registro a editar
   ...
   * @param {number} documentoId - ID del registro a eliminar
  ...
  onDelete(documentoId: number) {
    ...
    const shouldProceed = this.preDelete(documentoId);
    if (!shouldProceed) {
      return;
    }
    try {
  ...
  exportToExcel(): void {
    try {
      ...
      XLSX.writeFile(wb, 'documento_datos.xlsx');

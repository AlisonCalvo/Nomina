/**
 * @fileoverview Componente de eliminación para la entidad Proyecto
 */
...
/**
 * Componente para la eliminación de registros de Proyecto
 */
@Component({
...
export class EliminarProyectoComponent implements OnInit {
  /** Lista de registros de Proyecto */
  ...
  displayedColumns: string[] = ['estado', 'persona', 'alcanceContractual', 'numeroContrato', 'tiempoContractual', 'nombre', 'fechaFin', 'valorContrato', 'cliente', 'objetoContractual', 'creador', 'fechaInicio', 'id', 'actions'];
  ...
  private initializeFields() {
    this.proyectoService.findAll().subscribe(
      data => {
        // @ts-ignore
        this.proyectos = data.map(item => Object.assign(new ProyectoComponent(), item));
      },
      error => {

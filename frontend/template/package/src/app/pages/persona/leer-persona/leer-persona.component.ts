...
@Component({
  ...
  styleUrls: ['./leer-persona.component.scss']
})
export class LeerPersonaComponent implements OnInit {
   ...
   * @param personaService Servicio para gestionar la entidad
   ...
   * @param {any} persona - El registro a editar
   ...
   * @param {number} personaId - ID del registro a eliminar
  ...
  onDelete(personaId: number) {
    ...
    const shouldProceed = this.preDelete(personaId);
    if (!shouldProceed) {
      return;
    }
    try {
  ...
  exportToExcel(): void {
    try {
      ...
      XLSX.writeFile(wb, 'persona_datos.xlsx');

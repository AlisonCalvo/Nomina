...
@Component({
  ...
  styleUrls: ['./leer-tipocontrato.component.scss']
})
export class LeerTipoContratoComponent implements OnInit {
  // Columnas que se mostrarán en la tabla
  displayedColumns: string[] = ['id', 'nombreTipoContrato', 'creador', 'acciones'];
   ...
   * @param tipocontratoService Servicio para gestionar la entidad
  ...
  onCreate(): void {
    ...
    const dialogRef = this.dialog.open(CrearTipoContratoComponent, {
      minWidth: minWidth,
      data: {}
    });
   ...
   * @param {any} tipocontrato - El registro a editar
   ...
   * @param {number} tipocontratoId - ID del registro a eliminar
  ...
  onDelete(tipocontratoId: number) {
    ...
    const shouldProceed = this.preDelete(tipocontratoId);
    if (!shouldProceed) {
      return;
    }
    try {
      ...
      this.tipocontratoService.deleteById(tipocontratoId).subscribe(
        () => {
          ...
          this.postDelete(tipocontratoId);
        },
        error => {
  ...
  exportToExcel(): void {
    try {
      ...
      XLSX.writeFile(wb, 'tipocontrato_datos.xlsx');

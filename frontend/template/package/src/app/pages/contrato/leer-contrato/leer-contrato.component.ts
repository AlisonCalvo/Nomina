...
export class LeerContratoComponent implements OnInit {
   ...
   * @param contratoService Servicio para gestionar la entidad
   ...
   * @param {number} contratoId - ID del registro a eliminar
  ...
  onDelete(contratoId: number) {
    ...
    const shouldProceed = this.preDelete(contratoId);
    if (!shouldProceed) {
      return;
    }
    try {
  ...
  exportToExcel(): void {
    try {
      ...
      XLSX.writeFile(wb, 'contrato_datos.xlsx');
